...
# Updated to match SPAR's location-info schema
...
def search_and_select_store(driver, address):
    ...
    try:
        ...
        try:
            ...
            print("   -> Click timed out; sending Down Arrow + Enter for keyboard selection...")
        ...
        WebDriverWait(driver, WAIT_TIME_SECONDS).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, ALL_STORE_OPTIONS_SELECTOR))
        )
        ...
        for retry in range(3):
            try:
                parent_node = driver.find_element(By.CSS_SELECTOR, LOCATION_LIST_PARENT)
                store_options = parent_node.find_elements(By.CSS_SELECTOR, ALL_STORE_OPTIONS_SELECTOR)
                ...
                for idx, option in enumerate(store_options):
                    try:
                        address_text = option.find_element(By.CSS_SELECTOR, STORE_ADDRESS_RELATIVE_SELECTOR).text.strip()
                        if street_keyword.lower() in address_text.lower():
                            target_index = idx
                            print(f"   -> Found exact address match in results at index {idx}: '{address_text}'")
                            break
        ...
        for attempt in range(5):
            try:
                ...
                try:
                    store_name = target_option.find_element(By.CSS_SELECTOR, STORE_TITLE_RELATIVE_SELECTOR).text.strip()
                except NoSuchElementException:
                    store_name = "Target Store"

                button_to_click = target_option.find_element(By.CSS_SELECTOR, STORE_BUTTON_RELATIVE_SELECTOR)
                driver.execute_script("arguments[0].click();", button_to_click)
                print(f"   -> Clicked 'Markt wählen' for: '{store_name}'. Committing state...")
                break
            except (ElementClickInterceptedException, StaleElementReferenceException, IndexError):
                time.sleep(0.5)

        # 6. Wait for overlay modal to close
        try:
            WebDriverWait(driver, 10).until(
                EC.invisibility_of_element_located((By.CSS_SELECTOR, OVERLAY_WRAPPER_SELECTOR))
            )
            print("   -> Location overlay closed successfully.")
        except TimeoutException:
            print("   -> Warning: Overlay did not close automatically.")

        time.sleep(2)  # Allow session state/cookies to settle

        # 7. Verify UI header component matches chosen market
        selected_title_element = WebDriverWait(driver, 10).until(
            EC.presence_of_element_located((By.CSS_SELECTOR, SELECTED_STORE_HEADER_SELECTOR))
        )
        ...
        print(f"   -> Failed during store selection flow: {type(e).__name__}: {e}. FAILED. ❌")
        return False
...
def get_total_pages(driver):
    """Parses pagination text (e.g., '1 von 13') to determine total page count."""
    try:
...
def parse_product_card(card):
    """Extracts product attributes from a single SPAR product tile."""
...
def scrape_spar_offers(url, target_address):
    ...
    if click_store_select_button(driver) and search_and_select_store(
        ...
        print(
            "   -> WARNING: Failed to set store location. Proceeding with default location. ⚠️"
        )
    ...
    try:
        print(
            f"\nSuccessfully scraped {final_count} promotional products from SPAR (Localized data)."
        )
